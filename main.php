...
if(strlen($row["$broj"]["name"])>=20){
$row["$broj"]["name"] = substr($row["$broj"]["name"], 0, 20);
...
if($_GET["page"]=="podrska"){
	...
	if($_POST["otvori_tiket"] || $_POST["tiket_title"] || $_POST["pitanje"]){
		...
		$datum = time();
		$otvori_tiket = mysql_query('INSERT INTO tiketi (server_id, user_id, status, datum, naslov) VALUES ("'.$server.'", "'.$_SESSION["id"].'", "Neodgovoren", "'.$datum.'", "'.$tiket_naslov.'")');	
