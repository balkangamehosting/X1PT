...
$message = '<div class="notif success bloc">
    Napomena je uspješno spremljena. 
...
$subject = "X1PT Hosting - Game Panel Informacije";
$header = "From: $from";
...
Kako dodati admina i sve ostale informacije neophodne za upravljanje vasim serverom mozete naci na forumu u kategoriji (Baza Znanja)
Forum: http://forum.x1pt.info

Vas server ce biti aktivan onoliko vremena na kolko je uplacen,u Game Panel-u vam se nalazi vrjeme do kada je vas server aktivan.
Nakon isteka servera,server ostaje aktivan jos 2 dana.Ukoliko ni tada nebude placen server se automacki suspenduje.

Nakon suspendovanja vas server ce se i dalje nalaziti u Game Panel-u jos 8 dana ali nece biti aktiva,ukoliko se server i tada ne produzi bice izbrisan a ip ce nasljediti drugi korisnik

Za sva pitanja kontaktirajte nas na email: info@x1pt.info
...
$to = $email;	
$from = $admin_info["info@x1pt.info"];
$subject = "X1PT Hosting - Novi password";
$header = "From: $from";
$message = 'Postovani '.$fname.' '.$lname.'

Vas password je uspjesno restartovan.

Podaci za Game Panel su sljedeci:

Email: '.$to.'
Lozinka: '.$password.'
Game Panel '.$link.'

Ukoliko imate bilo kakvih pitanja ili problema mozete nas kontaktirati na info@x1pt.info
...
if($_POST["dodaj_radnika"]){
   ...
   if($_POST["email"]){
       ...
       if($username==0){
          ...
          $status = $_POST["status"];
		  if($status=="support"){
			$update_profile = mysql_query('INSERT INTO admin (fname, lname, username, password, email, status, napomena, boja) VALUES ("'.$ime.'", "'.$prezime.'", "'.$username.'", "'.$password.'", "'.$email.'", "'.$status.'", "Dobrodosao na x1pt host! :)", "#0A93CC")');
		  }else{
			$update_profile = mysql_query('INSERT INTO admin (fname, lname, username, password, email, status, napomena, boja) VALUES ("'.$ime.'", "'.$prezime.'", "'.$username.'", "'.$password.'", "'.$email.'", "'.$status.'", "Dobrodosao na x1pt host! :)", "red")');		  
		  }
...
if($_POST["mass_email"]){
    ...
    if($_POST["korisnici"]){
        ...
        if($_POST["naslov"]){
			...
			$headers .= 'From: serveri@x1pt.info X1PT Hosting' . "\r\n";
